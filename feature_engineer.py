...
def feature_engineering(data,num=5):
    ...
    for level in range(count):
        ...
        delVA = pl.when(cond1 | pl.col(f'askRate{level}').shift().is_null()).then(0).otherwise(pl.when(cond2).then(pl.col(f'askSize{level}').diff()).otherwise(pl.col(f'askSize{level}'))).alias('delVA')
        ...
        # Calculate the new column values based on the conditions
        delVB = pl.when(cond1 | pl.col(f'bidRate{level}').shift().is_null()).then(0).otherwise(pl.when(cond2).then(pl.col(f'bidSize{level}').diff()).otherwise(pl.col(f'bidSize{level}'))).alias('delVB')
        
        mkd1 = np.sqrt(pl.sum_horizontal([pl.col(f'askSize{i}') for i in range(level + 1)]) + pl.sum_horizontal([pl.col(f'bidSize{i}') for i in range(level + 1)]))
    ...
    for i in range(count):
        data = data.with_columns(
            [
                ((pl.col(f'askRate{i}') + pl.col(f'bidRate{i}') - 2 * pl.col(f'tmp.smart_mid_price0')) / (pl.col(f'askRate{i}') - pl.col(f'bidRate{i}')) ).alias(f'fea.PP{i}')
            ]
        )
    ...
    for i in [5, 10, 50, 90, 100, 110, 150, 300]:
        ...
        data = data.with_columns(
                pl.when(pl.col(f'tmp.mid_price0')<0).then(-pl.col(f'target.forw_ret{i}')).otherwise(pl.col(f'target.forw_ret{i}')).alias(f'target.forw_ret{i}')
        )
...
def normal_test_feature(df, feature, train_q_l, train_q_r, train_std, train_min, train_max):
    q001 = train_q_l
    q999 = train_q_r
