...
class transMODEL(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fc(out).squeeze()  # 取最后一个时间步的输出作为预测
...
class lgbModule(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.from_numpy(pred).squeeze().to(x.device) * self.model_weight
...
def loading_transformer(file_name, args):
    ...
    model.load_state_dict(torch.load(file_name), False)
    return model
...
class AveragingEnsemble(nn.Module):
    def __init__(self, ensemble_models, ensemble_ckpt, args, device, 
                 ensemble_style, manual_weight) -> None:
        super().__init__()
        self.models = {mm: LOADING_FUNC_MAPPING[mm](mp, args) for mm, mp in zip(ensemble_models, ensemble_ckpt)}
        self.models = {mm: self.models[mm].to(device) if hasattr(self.models[mm], 'to') else self.models[mm] for mm in self.models}
        self.ensemble_style = ensemble_style
        if ensemble_style == 'manual_weight':
            manual_weight = np.mean(manual_weight).tolist()
            self.manual_weight = {mm: mw for mm, mw in zip(ensemble_models, manual_weight)}
    ...
    def forward(self, x):
        ...
        ret = None
        if self.ensemble_style == 'manual_weight':
...
def get_models(ensemble_models, ensemble_ckpt, args, device):
    models = {mm: LOADING_FUNC_MAPPING[mm](mp, args) for mm, mp in zip(ensemble_models, ensemble_ckpt)}
